...
from lightrag.llm.openai import openai_completion, openai_complete_if_cache
...
async def _get_vector_context(
    ...
    """
    try:
        logger.debug(f"Starting vector search for query: {query}")
        ...
        for i, result in enumerate(results):
            logger.debug(f"Processing result {i}: {list(result.keys())}")
            if "content" in result:
                ...
                logger.warning(f"Result {i} missing 'content' field: {result}")

        logger.debug(f"Found {len(valid_chunks)} valid chunks")
        if not valid_chunks:
            logger.debug("No valid chunks found")
            return [], [], []

        # Ensure max_token_for_text_unit has a default value if None
        max_token_size = query_param.max_token_for_text_unit
        logger.debug(f"Original max_token_size: {max_token_size}")
        if max_token_size is None:
            max_token_size = 4000
        
        # Ensure max_token_size is positive
        if max_token_size <= 0:
            max_token_size = 4000
        
        logger.debug(f"Final max_token_size: {max_token_size}")
        ...
        logger.debug(
            f"Truncate chunks from {len(valid_chunks)} to {len(maybe_trun_chunks)} (max tokens:{max_token_size})"
        )
        logger.info(
        ...
        if not maybe_trun_chunks:
            logger.debug("No chunks after truncation")
            return [], [], []
        ...
        logger.debug(f"Returning {len(text_units_context)} text units")
...
async def naive_query(
    ...
    args_hash = compute_args_hash(
        query, asdict(query_param), "naive_query", system_prompt
    )
    cached_response, _, _, _ = await handle_cache(
        hashing_kv, args_hash, query, "naive_query", "query"
    )
    if cached_response:
        return cached_response

    # Vector search
    logger.info("Executing aquery with mode: naive")
    vector_result = await _get_vector_context(
        query, chunks_vdb, query_param, global_config["tokenizer"]
    )
    
    if vector_result is None:
        context_list = []
    else:
        _, _, context_list = vector_result
    ...
    if not context_list:
        final_prompt = PROMPTS["fail_response"].format(
            query=query, language=language
        )
        final_system_prompt = None
    else:
        # Process the context list - context_list is already a list of dictionaries
        # Convert each dictionary to a string format for the prompt
        context_strings = []
        for i, item in enumerate(context_list):
            content = item.get("content", "")
            file_path = item.get("file_path", "unknown_source")
            context_strings.append(f"[{i+1}] {content}\n(Source: {file_path})")
        
        context_str = "\n\n".join(context_strings)
        
        # Then, format the final prompt using the combined context string.
        final_system_prompt = PROMPTS["rag_response"].format(
            context_data=context_str,
            history=history_context,
            response_type=query_param.response_type or "Multiple Paragraphs",
            user_prompt=query_param.user_prompt or PROMPTS["DEFAULT_USER_PROMPT"],
        )
        final_prompt = query # The user query is the main prompt

    # Final answer generation
    # Prepare parameters for the original openai_complete_if_cache function
    llm_kwargs = global_config.get("llm_model_kwargs", {})
    completion_params = {
        "model": global_config.get("llm_model_name"),
        "prompt": final_prompt,
        "system_prompt": final_system_prompt,
        "stream": query_param.stream,
        "temperature": getattr(query_param, "temperature", None)
        or llm_kwargs.get("temperature"),
        "max_tokens": getattr(query_param, "max_tokens", None)
        or llm_kwargs.get("max_tokens"),
        "top_p": getattr(query_param, "top_p", None) or llm_kwargs.get("top_p"),
        "base_url": os.environ.get("LLM_BINDING_HOST"),
        "api_key": os.environ.get("LLM_BINDING_API_KEY"),
    }
    
    # Add stop parameter if provided
    stop_param = getattr(query_param, "stop", None)
    if stop_param:
        completion_params["stop"] = stop_param

    # Filter out None values before calling the function
    completion_params = {k: v for k, v in completion_params.items() if v is not None}

    logger.info(f"OpenAI API Call Params: {completion_params}")

    # Use the original openai_complete_if_cache function
    # Extract required positional parameters
    prompt = completion_params.pop("prompt")
    model = completion_params.pop("model")
